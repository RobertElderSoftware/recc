...
void test_regex_compile_error(struct memory_pool_collection * m, const char * c, enum regex_compiler_error_type type){
	...
	regex_compiler_state_explain(&compiler_state);
...
int do_test_case(struct memory_pool_collection * m, unsigned int num_regexes, const char ** regexes, unsigned int * should_match, unsigned int * match_length, const char * subject){
	...
	for(i = 0; i < num_regexes; i++){
		...
		regex_compiler_state_explain(compiler_states[i]);
		...
		for(j = 0; j < matcher.num_unique_matches_found; j++){
			...
			if(match_id == i){ /*  If this match belongs to the regex we want to check for */
				if(should_match[match_id]){
					...
					printf("Pattern '%s' matched, but shouldn't have %d.\n", regexes[match_id], match_id);
					cleanup_test_case(num_regexes, compiler_states, &matcher);
					return 1;
				}
...
const char * regexes1[4] =      {"b?a?a?a?baaa", "baaa", "foo", "ba[^a-zA-Z0-9_]"};
unsigned int does_match1[4] =   {             1,      1,     0,                 0};
unsigned int match_length1[4] = {             4,      4,     0,                 0};

void test_1(struct memory_pool_collection * m){
	assert(!do_test_case(m, 4, regexes1, does_match1, match_length1, "baaa"));
}

const char * regexes2[3] =      {"b?", "", "a"};
unsigned int does_match2[3] =   {   1,  1,   0};
unsigned int match_length2[3] = {   0,  0,   0};

void test_2(struct memory_pool_collection * m){
	assert(!do_test_case(m, 3, regexes2, does_match2, match_length2, ""));
}

const char * regexes3[8] =      {"[\\x00-\\xfF]", "[a-f]", "[a]", "[k]", "[k-zt-u]", "[k-za-u]+",      "[.]+",      ".+"};
unsigned int does_match3[8] =   {              1,       1,     1,     0,          0,           1,           1,         1};
unsigned int match_length3[8] = {              1,       1,     1,     0,          0,           3,           3,         3};

void test_3(struct memory_pool_collection * m){
	assert(!do_test_case(m, 8, regexes3, does_match3, match_length3, "abc"));
}

const char * regexes4[2] =      {".\\n\\t\\(\\)\\[\\]\\+\\?\\*\\.\\-\\^\\|\\\\x", ".[\\n\\t\\(\\)\\[\\]\\+\\?\\*\\.\\-\\^\\|\\\\x]+"};
unsigned int does_match4[2] =   {                                               1,                                                   1};
unsigned int match_length4[2] = {                                              16,                                                  16};

void test_4(struct memory_pool_collection * m){
	assert(!do_test_case(m, 2, regexes4, does_match4, match_length4, ".\n\t()[]+?*.-^|\\x"));
}

const char * regexes5[1] =       {"<[^ ]+ [^ ]+ *= *\"[^\"]*\" *>"};
unsigned int does_match5[1] =    {                               1};
unsigned int does_not_match5[1] ={                               0};
unsigned int match_length5[1] =  {                              15};
...
const char * regexes6[1] =      {"a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?a?"};
unsigned int does_match6[1] =   {                                1};
unsigned int match_length6[1] = {                              100};
...
const char * regexes7[8] =      {"(dog|foo)", "(foo|cat|dog)", "(foo|dog|cat)", "(|dog)", "(dog|)", "(dogen|def)",      "(abc|def)",      "(abc|dogen)"};
unsigned int does_match7[8] =   {          1,               1,               1,     1,          1,              0,                0,                  0};
unsigned int match_length7[8] = {          3,               3,               3,     3,          3,              0,                0,                  0};

void test_7(struct memory_pool_collection * m){
	assert(!do_test_case(m, 8, regexes7, does_match7, match_length7, "dogasdf"));
}

const char * regexes8[10] =      {"aaa(lol)", "aaa(lol)f", "aaa(lol)f?", "aaa(lol)f+", "aaa(lol)f.", "aaa((l)ol)f", "aaa(l(o)l)f","aaa(lol(f))f", "aaa((l))o", "aaa((l))"};
unsigned int does_match8[10] =   {         1,           1,            1,            1,            1,              1,            1,             0,           1,          1};
unsigned int match_length8[10] = {         6,           7,            7,            7,            8,              7,            7,             0,           5,          4};
...
const char * sample_c = "int main(void) { /* A comment* */ unsigned int abc = 0; unsigned int def = 0; return abc==def; } /* A comment \n* \n */ auto break case char const continue default do double else enum extern float for goto if int long register return short signed sizeof static struct switch typedef union unsigned void volatile while foobar 0xfabe] 4E2 .99 4.4f 33 'a' \"a\\\"\" # \"d\" ... >>= <<= += -= *= /= %= &= ^= |= >> << ++ -- -> && || <= >= == != ; { } [ ] , : = ( ) . & ! ~ - + * / % < > ^ | ? #";

#define NUM_EXPECTED_TYPES 116

enum regex_compiler_error_type expected_types[NUM_EXPECTED_TYPES] = {EX_INT, EX_IDENTIFIER, EX_OPEN_PAREN_CHAR, EX_VOID, EX_CLOSE_PAREN_CHAR, EX_OPEN_BRACE_CHAR, EX_COMMENT, EX_UNSIGNED, EX_INT, EX_IDENTIFIER, EX_EQUALS_CHAR, EX_CONSTANT_DECIMAL, EX_SEMICOLON_CHAR, EX_UNSIGNED, EX_INT, EX_IDENTIFIER, EX_EQUALS_CHAR, EX_CONSTANT_DECIMAL, EX_SEMICOLON_CHAR, EX_RETURN, EX_IDENTIFIER, EX_EQ_OP, EX_IDENTIFIER, EX_SEMICOLON_CHAR, EX_CLOSE_BRACE_CHAR, EX_COMMENT, EX_AUTO, EX_BREAK, EX_CASE, EX_CHAR, EX_CONST, EX_CONTINUE, EX_DEFAULT, EX_DO, EX_DOUBLE, EX_ELSE, EX_ENUM, EX_EXTERN, EX_FLOAT, EX_FOR, EX_GOTO, EX_IF, EX_INT, EX_LONG, EX_REGISTER, EX_RETURN, EX_SHORT, EX_SIGNED, EX_SIZEOF, EX_STATIC, EX_STRUCT, EX_SWITCH, EX_TYPEDEF, EX_UNION, EX_UNSIGNED, EX_VOID, EX_VOLATILE, EX_WHILE, EX_IDENTIFIER, EX_CONSTANT_HEX, EX_CLOSE_SQUARE_BRACKET_CHAR, EX_CONSTANT_EXPONENT, EX_CONSTANT_FLOAT_SMALL, EX_CONSTANT_FLOAT_LARGE, EX_CONSTANT_DECIMAL, EX_CONSTANT_CHARACTER, EX_STRING_LITERAL, EX_NUMBER_SIGN_CHAR, EX_STRING_LITERAL, EX_ELLIPSIS, EX_RIGHT_ASSIGN, EX_LEFT_ASSIGN, EX_ADD_ASSIGN, EX_SUB_ASSIGN, EX_MUL_ASSIGN, EX_DIV_ASSIGN, EX_MOD_ASSIGN, EX_AND_ASSIGN, EX_XOR_ASSIGN, EX_OR_ASSIGN, EX_RIGHT_OP, EX_LEFT_OP, EX_INC_OP, EX_DEC_OP, EX_PTR_OP, EX_AND_OP, EX_OR_OP, EX_LE_OP, EX_GE_OP, EX_EQ_OP, EX_NE_OP, EX_SEMICOLON_CHAR, EX_OPEN_BRACE_CHAR, EX_CLOSE_BRACE_CHAR, EX_OPEN_SQUARE_BRACKET_CHAR, EX_CLOSE_SQUARE_BRACKET_CHAR, EX_COMMA_CHAR, EX_COLON_CHAR, EX_EQUALS_CHAR, EX_OPEN_PAREN_CHAR, EX_CLOSE_PAREN_CHAR, EX_DOT_CHAR, EX_AMPERSAND_CHAR, EX_EXCLAMATION_MARK_CHAR, EX_TILDE_CHAR, EX_MINUS_CHAR, EX_PLUS_CHAR, EX_MULTIPLY_CHAR, EX_DIVIDE_CHAR, EX_PERCENT_CHAR, EX_OPEN_ANGLE_BRACKET_CHAR, EX_CLOSE_ANGLE_BRACKET_CHAR, EX_CARET_CHAR, EX_PIPE_CHAR, EX_QUESTION_MARK_CHAR, EX_NUMBER_SIGN_CHAR};
...
void mini_lexer_test(struct memory_pool_collection * m){
	...
	while(offset < len){
		...
		unsigned int j;
		regex_matcher_state_run(&matcher, (unsigned char *)&sample_c[offset], len - offset);
		...
		printf("'.\n");
		offset += match_len;
		if(current_index < NUM_EXPECTED_TYPES){
			...
			printf("Got %s after tests.\n", example_token_names[match_id]);
		}
	}
