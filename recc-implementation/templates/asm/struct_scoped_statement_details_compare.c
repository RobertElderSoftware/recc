...
int T0_IDENTIFIER_compare(T0_LITERAL * a, T0_LITERAL * b){
	if(a->key.type > b->key.type){
		...
		if(a->key.any_statement_list_item_id.id > b->key.any_statement_list_item_id.id){
			return 1;
		}else if(a->key.any_statement_list_item_id.id < b->key.any_statement_list_item_id.id){
			return -1;
		}else{
