...
void add_global_primative_to_list(struct code_gen_state *, struct type_description *, struct constant_initializer_level *, struct struct_compile_time_constant_ptr_list *, struct struct_type_description_ptr_list *);
void process_global_type(struct code_gen_state *, struct struct_compile_time_constant_ptr_list *, struct struct_type_description_ptr_list *);
...
void add_linker_object(struct code_gen_state * state, unsigned char * start, unsigned char * end, enum l2_token_type type){
	...
	new_object->start_label = start;
	new_object->end_label = end;
	struct_linker_object_ptr_list_add_end(&state->object_declarations, new_object);
}

unsigned int evaluate_compile_time_constant(struct code_gen_state * state, struct compile_time_constant * constant){
	unsigned int rtn;
	(void)state;
	if(constant->constant_description){
		rtn = constant->constant_description->native_data[0];
	}else if(constant->element){
		rtn = get_enum_value(state->memory_pool_collection, constant->element);
	}else{
		assert(0 && "Error.");
	}
	return rtn;
...
struct compile_time_constant * evaluate_constant_sizeof_type_name(struct code_gen_state * state, struct parser_node * n){
	...
	void_ptr_list_add_end(&state->created_data, (void*)rtn->constant_description->native_data);
	...
	*(rtn->constant_description->native_data) = size;
	...
	rtn->constant_description->type_description->source_scope_level = state->current_scope;
	destroy_type_description(state->memory_pool_collection, sized_type);
	return rtn;
}

struct compile_time_constant * evaluate_constant_literal(struct code_gen_state * state, struct parser_node * n){
	struct compile_time_constant * rtn;
...
struct compile_time_constant * evaluate_constant_identifier(struct code_gen_state * state, struct parser_node * n){
	struct compile_time_constant * rtn;
...
struct compile_time_constant * evaluate_constant_constant_expression(struct code_gen_state * state, struct parser_node * n){
	assert(n && n->type == CONSTANT_EXPRESSION);
	return evaluate_constant_conditional_expression(state, n->first_child);
}

struct compile_time_constant * evaluate_constant_primary_expression(struct code_gen_state * state, struct parser_node * n){
	assert(n->type == PRIMARY_EXPRESSION);
	if(n->first_child->type == TERMINAL && n->first_child->c_lexer_token->type == STRING_LITERAL){
		return evaluate_constant_literal(state, n->first_child);
...
struct compile_time_constant * evaluate_constant_postfix_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_unary_expression(struct code_gen_state * state, struct parser_node * n){
	assert(n->type == UNARY_EXPRESSION);
	if(n->first_child && n->first_child->type == POSTFIX_EXPRESSION){
		return evaluate_constant_postfix_expression(state, n->first_child);
...
struct compile_time_constant * evaluate_constant_cast_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_multiplicative_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_additive_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_shift_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_relational_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_equality_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_and_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_exclusive_or_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_inclusive_or_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_logical_and_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_logical_or_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_conditional_expression(struct code_gen_state * state, struct parser_node * n){
...
struct compile_time_constant * evaluate_constant_assignment_expression(struct code_gen_state * state, struct parser_node * n){
...
void evaluate_constant_initializer_list(struct code_gen_state * state, struct constant_initializer_level * parent_level, struct parser_node * n){
	...
	struct_constant_initializer_level_ptr_list_add_end(&parent_level->children, evaluate_constant_initializer(state, initializer));
	if(initializer->next->first_child->type != EPSILON){
		evaluate_constant_initializer_list(state, parent_level, initializer->next);
	}
}

struct constant_initializer_level * evaluate_constant_initializer(struct code_gen_state * state, struct parser_node * n){
	...
	struct_constant_initializer_level_ptr_list_add_end(&state->created_constant_initializer_levels, level);
	...
	if(n->first_child && n->first_child->type == ASSIGNMENT_EXPRESSION){
		level->constant = evaluate_constant_assignment_expression(state, n->first_child);
		...
		evaluate_constant_initializer_list(state, level, n->first_child->next);
	}else{
...
void perform_integral_promotion(struct code_gen_state * state, struct type_description ** t, const char * reg){
	...
	switch(c){
		case TYPE_CLASS_CHAR:{
			...
			add_specifier(state->memory_pool_collection, *t, INT);
			break;
		}case TYPE_CLASS_SHORT:{
			remove_specifier(state->memory_pool_collection, *t, 0, SHORT);
			if(!count_specifiers(*t,INT)){
				add_specifier(state->memory_pool_collection, *t, INT);
			}
			break;
		}case TYPE_CLASS_ENUM:{
			remove_enum(state->memory_pool_collection, *t);
			add_specifier(state->memory_pool_collection, *t, INT);
			break;
...
struct type_description * usual_arithmetic_conversion(struct code_gen_state * state, struct parser_node * context){
	...
	perform_integral_promotions(state, &t1, &t2);

	/*  These may have changed after the integer type promotion */
	c1 = determine_type_class(state->memory_pool_collection, t1);
	c2 = determine_type_class(state->memory_pool_collection, t2);
...
unsigned int struct_type_size(struct code_gen_state * state, struct type_description * t, enum value_type tsc, struct scope_level * source_scope_level){
	...
	if(!obj){
		printf("Unknown structure: %s in file %s.\n", tag_identifier, state->parser_state->c_lexer_state->c.filename);
		assert(obj && "Unknown identifier in struct_type_size.");
	}
	heap_memory_pool_free(state->memory_pool_collection, tag_identifier);
...
void move_pointers_down(struct code_gen_state * state, const char * ra, const char * rb){
	buffered_printf(state->buffered_output, "sub %s %s WR;\n", ra, ra);
	buffered_printf(state->buffered_output, "sub %s %s WR;\n", rb, rb);
}
	...
	switch(traversal->type_class){
		case TYPE_CLASS_POINTER:{
			...
			if(constant_expression){
				...
				traversal->arity = evaluate_compile_time_constant(state, evaluate_constant_constant_expression(state, constant_expression));
			}else{
...
void process_global_type(struct code_gen_state * state, struct struct_compile_time_constant_ptr_list * constants, struct struct_type_description_ptr_list * types){
	...
	for(i = 0; i < num_constants; i++){
		...
		if(c && c->constant_description && c->constant_description->type == STRING_LITERAL){
			...
			heap_memory_pool_free(state->memory_pool_collection, string_literal_identifier_str);
		}else{
			...
			switch(element_size){
				case 1:{
					...
					bytes_buffered += 1;
					if(bytes_buffered == 4){
						buffered_printf(state->buffered_output, "DW 0x%X;\n", current_word);
						bytes_buffered = 0;
						current_word = 0;
					}
					break;
	...
	/*  Not enough bytes to complete a whole word */
	if(bytes_buffered){
		buffered_printf(state->buffered_output, "DW 0x%X;\n", current_word);
	}
}

void setup_global_type(struct code_gen_state * state, struct type_traversal * type_traversal, struct constant_initializer_level * initializer_level, struct struct_compile_time_constant_ptr_list * constants, struct struct_type_description_ptr_list * types){
	switch(type_traversal->type_class){
		case TYPE_CLASS_POINTER:{
			...
			for(i = 0; i < type_traversal->arity; i++){
				...
				setup_global_type(state, element_traversal, current_initializer_level, constants, types);
			}
			break;
		}case TYPE_CLASS_FUNCTION:{
			assert(0 && "Not implemented.");
			...
			for(i = 0; i < num_children; i++){
				...
				setup_global_type(state, member, (struct constant_initializer_level *)0, constants, types);
			}
			break;
		}case TYPE_CLASS_UNION:{
			assert(0);
...
void add_global_primative_to_list(struct code_gen_state * state, struct type_description * t, struct constant_initializer_level * current_initializer_level, struct struct_compile_time_constant_ptr_list * constants, struct struct_type_description_ptr_list * types){
...
unsigned int type_size(struct code_gen_state * state, struct type_description * t, enum value_type tsc, unsigned int force_arity_1, struct scope_level * source_scope_level){
	...
	if(c == TYPE_CLASS_STRUCT){
		return struct_type_size(state, t, tsc, source_scope_level);
	}else if(c == TYPE_CLASS_UNION){
		assert(0);
		return 8; /* Works sometimes */
	}else if(c == TYPE_CLASS_ENUM){
		return enum_type_size(state->memory_pool_collection, t, tsc);
	}else if(c == TYPE_CLASS_VOID){
		return void_type_size(state->memory_pool_collection, tsc);
	}else{
		return arithmetic_type_size(state->memory_pool_collection, t, tsc);
	}
}
...
unsigned int get_parameter_offset(struct code_gen_state * state, struct namespace_object * obj){
	if(obj){
		...
		if(is_array(state->memory_pool_collection, abstract_declarator)){
			...
			rtn = 4 + get_parameter_offset(state, obj->previous);
		}else{
			rtn = get_namespace_object_size(state, obj, 0) + get_parameter_offset(state, obj->previous);
		}
...
unsigned int get_local_offset(struct code_gen_state * state, struct namespace_object * obj){
	...
	of the outer variable.
        */
		...
		if(start_obj){
			unsigned int a;
			a = get_local_offset_h1(state, start_obj, force_arity_1);
...
unsigned int get_local_offset_h1(struct code_gen_state * state, struct namespace_object * obj, unsigned int force_arity_1){
	...
	if(obj && !obj->first_load){
		...
		buffered_printf(state->buffered_output, ";Add offset of %d due to local %s\n", a, identifier);
		heap_memory_pool_free(state->memory_pool_collection, identifier);
		return a + get_local_offset_h1(state, obj->next, 0);
	}else{
		return 0;
	}
}
...
unsigned int calculate_type_stack_size(struct code_gen_state * state){
	...
	for(i = 0; i < num_types; i++){
		...
		unsigned int current_type_size = type_size(state, t, t->value_type, 1, state->current_scope);
...
void delete_top_type(struct code_gen_state * state){
	...
	size = type_size(state, t, t->value_type, 1, state->current_scope);
	num_words = size / 4;
	assert(size - (4*num_words) == 0);
	for(i = 0; i < num_words; i++){
		buffered_printf(state->buffered_output,"add SP SP WR; Deleting top type word %d.\n", i);
	}
	destroy_type_description(state->memory_pool_collection, t);
}

void copy_words(struct code_gen_state * state, const char * src_addr, const char * dst_addr, const char * tmp, unsigned int bytes_to_copy){
	...
	for(i = 0; i < (words_to_copy -1); i++){
		...
		buffered_printf(state->buffered_output,"sto %s %s;\n", dst_addr, tmp);
		buffered_printf(state->buffered_output,"sub %s %s WR;\n", dst_addr, dst_addr);
		buffered_printf(state->buffered_output,"sub %s %s WR;\n", src_addr, src_addr);
	}
	buffered_printf(state->buffered_output,"loa %s %s;\n", tmp, src_addr);
...
void load_identifier(struct code_gen_state * state, unsigned char * identifier, struct parser_node * context){
	...
	switch(obj->object_location){
		case GLOBAL:{
			...
			unsigned int temps_size = calculate_type_stack_size(state);
			...
			if(obj->first_load){
				manage_local_space(state, obj, 1);
				obj->first_load = 0;
			}
...
void g_primary_expression(struct parser_node * p, struct code_gen_state * state){
	struct parser_state * parser_state = state->parser_state;
	struct memory_pool_collection * m = state->memory_pool_collection;
	if(check_three_children(p, TERMINAL, EXPRESSION, TERMINAL)){
		...
		if(is_terminal_c_token_type(first_child(p), CONSTANT_CHARACTER)){
			...
			heap_memory_pool_free(m, constant);
			...
			load_identifier(state, identifier, p);
			heap_memory_pool_free(m, identifier);
			...
			heap_memory_pool_free(m, string_literal_identifier_str);
			heap_memory_pool_free(m, constant);
		}else{
...
void copy_lvalue_into_rvalue(struct code_gen_state * state, const char * source_lvalue, const char * dst, const char * size_minus_4, const char * tmp, unsigned int size){
	buffered_printf(state->buffered_output,"add r3 r3 %s;     Copy from last word of rvalue\n", size_minus_4);
	copy_words(state, source_lvalue, dst, tmp, size);
}

void do_character_rvalue_conversion(struct code_gen_state * state, const char * lvalue, const char * dst, const char * tmp1, const char * tmp2, const char * tmp3){
	/*  Assumes that reg is pointing to an lvalue the represents the character */
	buffered_printf(state->buffered_output,"div %s %s WR;     stores int div amount\n", tmp3, lvalue);
	buffered_printf(state->buffered_output,"mul %s %s WR;     4 byte aligned address\n", tmp2, tmp3);
	buffered_printf(state->buffered_output,"loa %s %s;        stores the value with other bytes\n", tmp1, tmp2);
	buffered_printf(state->buffered_output,"sub %s %s %s;     mod amount\n", tmp2, lvalue, tmp2);
	buffered_printf(state->buffered_output,"ll %s 0xFF;       Byte mask\n", tmp3);
	buffered_printf(state->buffered_output,"mul %s WR %s;     Bytes to shift left\n", tmp2, tmp2);
	buffered_printf(state->buffered_output,"shl %s %s;        Shift mask to target byte\n", tmp3, tmp2);
	buffered_printf(state->buffered_output,"shl %s %s;        Shift mask to target byte\n", tmp3, tmp2);
	buffered_printf(state->buffered_output,"and %s %s %s;     Grab the bits we want\n", tmp1, tmp3, tmp1);
	buffered_printf(state->buffered_output,"shr %s %s;        Shift result to right\n", tmp1, tmp2);
	buffered_printf(state->buffered_output,"shr %s %s;        Shift result to right\n", tmp1, tmp2);
...
void compare_function_argument_type(struct code_gen_state * state, struct parser_node * parameter_declaration, unsigned int param_index, struct parser_node * context){
	...
	type_class_observed = determine_type_class(state->memory_pool_collection, top_type);
	type_class_required = determine_type_class(state->memory_pool_collection, param_type);

	/*  Normalize to pointers */
	decay_to_pointer_if_array(state, &top_type);
	decay_to_pointer_if_array(state, &param_type);
	...
	if(!type_description_cmp(state->memory_pool_collection, param_type, top_type)){
		/*  Put the parameter type back on the stack.*/
		push_type(state, top_type, context);
		destroy_type_description(state->memory_pool_collection, param_type);
	}else{
		print_error_with_types(state->parser_state->c_lexer_state, param_type, top_type, context, "\nType missmatch for argument.");
		printf("Missmatch from argument with zero based index %d.  \n", param_index);
		print_error_with_type(state->parser_state->c_lexer_state, param_type, parameter_declaration, "Function was declared here:");
		assert(0);
	}

}

unsigned int g_argument_expression_list_rest(struct parser_node * p, struct code_gen_state * state, struct parser_node * parameter_type_list, unsigned int current_parameter_index, unsigned int is_variadic){
	if(check_three_children(p, TERMINAL, ASSIGNMENT_EXPRESSION, ARGUMENT_EXPRESSION_LIST_REST)){
		if(is_terminal_c_token_type(first_child(p),COMMA_CHAR)){
			...
			if(is_variadic){
				...
				compare_function_argument_type(state, parameter_declaration, current_parameter_index, second_child(p));
			}else{
				print_error_with_type(state->parser_state->c_lexer_state, arg_type, second_child(p), "Too many parameters.");
				assert(0 && "Too many parameters.");
			}
			return arg_type_size + g_argument_expression_list_rest(third_child(p), state, parameter_type_list, current_parameter_index + 1, is_variadic);
		}else{
...
unsigned int g_argument_expression_list(struct parser_node * p, struct code_gen_state * state, struct parser_node * parameter_type_list, unsigned int current_parameter_index, unsigned int is_variadic){
	if(check_two_children(p, ASSIGNMENT_EXPRESSION, ARGUMENT_EXPRESSION_LIST_REST)){
		...
		if(is_variadic){
			...
			compare_function_argument_type(state, parameter_declaration, current_parameter_index, first_child(p));
		}else{
			print_error_with_type(state->parser_state->c_lexer_state, arg_type, first_child(p), "Too many parameters.");
			assert(0 && "Too many parameters.");
		}
		return arg_type_size + g_argument_expression_list_rest(second_child(p), state, parameter_type_list, current_parameter_index + 1, is_variadic);
	}else{
...
void function_call(struct parser_node * argument_expression_list, struct parser_node * postfix_experssion_rest, struct code_gen_state * state, struct parser_node * context){
	...
	destroy_type_description(state->memory_pool_collection, pop_type(state, word_type, context));
	destroy_type_description(state->memory_pool_collection, fcn_ptr_type);
	destroy_type_description(state->memory_pool_collection, fcn_type);
	destroy_type_description(state->memory_pool_collection, word_type);
	g_postfix_expression_rest(postfix_experssion_rest, state);
	destroy_parser_node_tree_and_c_lexer_tokens(state->memory_pool_collection, abstract_declarator);
}

void do_struct_dereference_operator(struct code_gen_state * state, unsigned char * target_member_identifier, struct parser_node * context, unsigned int is_ptr){
	...
	struct_tag_identifier = struct_tag_token ? copy_string(struct_tag_token->first_byte, struct_tag_token->last_byte, state->memory_pool_collection) : make_up_identifier(state->parser_state, t->source_element, state->memory_pool_collection);
	...
	destroy_parser_node_tree_and_c_lexer_tokens(state->memory_pool_collection, abstract_declarator);
	heap_memory_pool_free(state->memory_pool_collection, struct_tag_identifier);
	destroy_type_description(state->memory_pool_collection, t);
}

void g_postfix_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, EXPRESSION, TERMINAL, POSTFIX_EXPRESSION_REST)){
		if(
			...
			g_postfix_expression_rest(fourth_child(p), state);
		}else{
		...
		if(
			...
			function_call(second_child(p), fourth_child(p), state, p);
		}else{
			assert(0 &&"Expected () in postfix expression rest.\n");
		}
		...
		if(
			...
			function_call((struct parser_node *)0, third_child(p), state, p);
		}else if(
			is_terminal_c_token_type(first_child(p), DOT_CHAR) &&
			is_terminal_c_token_type(second_child(p), IDENTIFIER)
		){
			unsigned char * identifier = copy_string(second_child(p)->c_lexer_token->first_byte, second_child(p)->c_lexer_token->last_byte, state->memory_pool_collection);
			do_struct_dereference_operator(state, identifier, second_child(p), 0);
			g_postfix_expression_rest(third_child(p), state);
			heap_memory_pool_free(state->memory_pool_collection, identifier);
		}else if(
			...
			unsigned char * identifier = copy_string(second_child(p)->c_lexer_token->first_byte, second_child(p)->c_lexer_token->last_byte, state->memory_pool_collection);
			do_struct_dereference_operator(state, identifier, second_child(p), 1);
			g_postfix_expression_rest(third_child(p), state);
			heap_memory_pool_free(state->memory_pool_collection, identifier);
		}else{
			assert(0 &&"Expected () in postfix expression rest.\n");
		...
		if(is_terminal_c_token_type(first_child(p), INC_OP)){
			...
			t->source_scope_level = state->current_scope;
...
void g_postfix_expression(struct parser_node * p, struct code_gen_state * state){
...
void g_unary_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, TERMINAL, TYPE_NAME, TERMINAL)){
		if(
			...
			result_type->source_scope_level = state->current_scope;
		...
		g_cast_expression(second_child(p), state);
...
void g_cast_expression(struct parser_node * p, struct code_gen_state * state){
	if(is_first_child_type(p, UNARY_EXPRESSION)){
		g_unary_expression(first_child(p), state);
		...
		if(
			...
			convert_to_untypedefed_type_description(state->memory_pool_collection, new_type);
			is_new_type_integral = is_integral_type(state->memory_pool_collection, new_type);

			g_cast_expression(fourth_child(p), state);

			old_type = pop_type_without_type_check(state, p);
...
void do_signed_operation_function_call(struct code_gen_state * state, const char * function_name){
...
void g_multiplicative_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, CAST_EXPRESSION, MULTIPLICATIVE_EXPRESSION_REST)){
		do_multiplicative_expression(state, p, p->first_child->c_lexer_token->type);

		if(is_second_child_type(third_child(p), CAST_EXPRESSION)){
			g_cast_expression(second_child(third_child(p)), state);
			g_multiplicative_expression_rest(third_child(p), state);
		}
...
void g_multiplicative_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, CAST_EXPRESSION, MULTIPLICATIVE_EXPRESSION_REST)){
		g_cast_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), CAST_EXPRESSION)){
			g_cast_expression(second_child(second_child(p)), state);
...
struct type_description * perform_pointer_conversion(struct code_gen_state * state, struct parser_node * context){
	...
	unsigned int is_integral_type1 = is_integral_type(state->memory_pool_collection, t1);
	unsigned int is_integral_type2 = is_integral_type(state->memory_pool_collection, t2);
...
void g_additive_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, MULTIPLICATIVE_EXPRESSION, ADDITIVE_EXPRESSION_REST)){
		do_additive_expression(state, p, p->first_child->c_lexer_token->type);
		if(is_second_child_type(third_child(p), MULTIPLICATIVE_EXPRESSION)){
			g_multiplicative_expression(second_child(third_child(p)), state);
			g_additive_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_additive_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, MULTIPLICATIVE_EXPRESSION, ADDITIVE_EXPRESSION_REST)){
		g_multiplicative_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), MULTIPLICATIVE_EXPRESSION)){
			g_multiplicative_expression(second_child(second_child(p)), state);
...
void g_shift_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, ADDITIVE_EXPRESSION, SHIFT_EXPRESSION_REST)){
		do_shift_expression(state, p, first_child(p)->c_lexer_token->type);
		if(is_second_child_type(third_child(p), ADDITIVE_EXPRESSION)){
			g_additive_expression(second_child(third_child(p)), state);
			g_shift_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_shift_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, ADDITIVE_EXPRESSION, SHIFT_EXPRESSION_REST)){
		g_additive_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), ADDITIVE_EXPRESSION)){
			g_additive_expression(second_child(second_child(p)), state);
...
void g_relational_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, SHIFT_EXPRESSION, RELATIONAL_EXPRESSION_REST)){
		...
		if(is_second_child_type(third_child(p), SHIFT_EXPRESSION)){
			g_shift_expression(second_child(third_child(p)), state);
			g_relational_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_relational_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, SHIFT_EXPRESSION, RELATIONAL_EXPRESSION_REST)){
		g_shift_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), SHIFT_EXPRESSION)){
			g_shift_expression(second_child(second_child(p)), state);
...
void g_equality_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, RELATIONAL_EXPRESSION, EQUALITY_EXPRESSION_REST)){
		...
		if(is_second_child_type(third_child(p), RELATIONAL_EXPRESSION)){
			g_relational_expression(second_child(third_child(p)), state);
			g_equality_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_equality_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, RELATIONAL_EXPRESSION, EQUALITY_EXPRESSION_REST)){
		g_relational_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), RELATIONAL_EXPRESSION)){
			g_relational_expression(second_child(second_child(p)), state);
...
void g_and_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, EQUALITY_EXPRESSION, AND_EXPRESSION_REST)){
		do_and_expression(state, p, first_child(p)->c_lexer_token->type);
		if(is_second_child_type(third_child(p), EQUALITY_EXPRESSION)){
			g_equality_expression(second_child(third_child(p)), state);
			g_and_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_and_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, EQUALITY_EXPRESSION, AND_EXPRESSION_REST)){
		g_equality_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), EQUALITY_EXPRESSION)){
			g_equality_expression(second_child(second_child(p)), state);
...
void g_exclusive_or_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, AND_EXPRESSION, EXCLUSIVE_OR_EXPRESSION_REST)){
		do_xor_expression(state, p, first_child(p)->c_lexer_token->type);

		if(is_second_child_type(third_child(p), AND_EXPRESSION)){
			g_and_expression(second_child(third_child(p)), state);
			g_exclusive_or_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_exclusive_or_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, AND_EXPRESSION, EXCLUSIVE_OR_EXPRESSION_REST)){
		g_and_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), AND_EXPRESSION)){
			g_and_expression(second_child(second_child(p)), state);
...
void g_inclusive_or_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, EXCLUSIVE_OR_EXPRESSION, INCLUSIVE_OR_EXPRESSION_REST)){
		do_or_expression(state, p, first_child(p)->c_lexer_token->type);
		if(is_second_child_type(third_child(p), EXCLUSIVE_OR_EXPRESSION)){
			g_exclusive_or_expression(second_child(third_child(p)), state);
			g_inclusive_or_expression_rest(third_child(p), state);
		}
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_inclusive_or_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, EXCLUSIVE_OR_EXPRESSION, INCLUSIVE_OR_EXPRESSION_REST)){
		g_exclusive_or_expression(first_child(p), state);
		if(is_second_child_type(second_child(p), EXCLUSIVE_OR_EXPRESSION)){
			g_exclusive_or_expression(second_child(second_child(p)), state);
...
void g_logical_and_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, INCLUSIVE_OR_EXPRESSION, LOGICAL_AND_EXPRESSION_REST)){
		...
		g_rest_of_logical_and(third_child(p), state);
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_rest_of_logical_and(struct parser_node * possible_non_empty_rest, struct code_gen_state * state){
	if(is_second_child_type(possible_non_empty_rest, INCLUSIVE_OR_EXPRESSION)){
		...
		push(state, "r1", WORD_ALIGNED_RVALUE);
		g_inclusive_or_expression(second_child(possible_non_empty_rest), state);
		g_logical_and_expression_rest(possible_non_empty_rest, state);
		buffered_printf(state->buffered_output,"%s:\n", afterand_str);
		require_internal_symbol(m, &state->symbols, dontshort_str);
		implement_internal_symbol(m, &state->symbols, dontshort_str);
		require_internal_symbol(m, &state->symbols, afterand_str);
		implement_internal_symbol(m, &state->symbols, afterand_str);
		heap_memory_pool_free(m, dontshort_str);
		heap_memory_pool_free(m, afterand_str);
	}
}

void g_logical_and_expression(struct parser_node * p, struct code_gen_state * state){
...
void g_logical_or_expression_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, LOGICAL_AND_EXPRESSION, LOGICAL_OR_EXPRESSION_REST)){
		...
		g_rest_of_logical_or(third_child(p), state);
	}else if(is_second_child_type(p,EPSILON)){
		/*  Nothing for epsilon */
	}else{
...
void g_rest_of_logical_or(struct parser_node * possible_non_empty_rest, struct code_gen_state * state){
	if(is_second_child_type(possible_non_empty_rest, LOGICAL_AND_EXPRESSION)){
		...
		buffered_printf(state->buffered_output,"%s:\n", dontshort_str);
		push(state, "r1", WORD_ALIGNED_RVALUE);
		g_logical_and_expression(second_child(possible_non_empty_rest), state);
		g_logical_or_expression_rest(possible_non_empty_rest, state);
		buffered_printf(state->buffered_output,"%s:\n", afteror_str);
		require_internal_symbol(m, &state->symbols, dontshort_str);
		implement_internal_symbol(m, &state->symbols, dontshort_str);
		require_internal_symbol(m, &state->symbols, afteror_str);
		implement_internal_symbol(m, &state->symbols, afteror_str);
		heap_memory_pool_free(m, dontshort_str);
		heap_memory_pool_free(m, afteror_str);
	}
}

void g_logical_or_expression(struct parser_node * p, struct code_gen_state * state){
...
void g_conditional_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_five_children(p, LOGICAL_OR_EXPRESSION, TERMINAL, EXPRESSION, TERMINAL, CONDITIONAL_EXPRESSION)){
		if(
			...
			t2 = ensure_top_type_is_r_value(state, p);
			buffered_printf(state->buffered_output,"%s:\n", after_condition_str);
			...
			heap_memory_pool_free(m, after_condition_str);
			heap_memory_pool_free(m, false_condition_str);
			if(!type_description_cmp(m, t1, t2)){
				push_type(state, t1, p);
        			destroy_type_description(m, t2);
			}else{
				print_node_context(state->parser_state->c_lexer_state, p);
...
void g_assignment_operator(struct parser_node * p, struct code_gen_state * state){
	...
	if(check_one_child(p, TERMINAL)){
		if(is_terminal_c_token_type(first_child(p),EQUALS_CHAR)){
			...
			do_multiplicative_expression(state, p, MULTIPLY_CHAR);
		}else if(is_terminal_c_token_type(first_child(p),DIV_ASSIGN)){
			do_multiplicative_expression(state, p, DIVIDE_CHAR);
		}else if(is_terminal_c_token_type(first_child(p),MOD_ASSIGN)){
			do_multiplicative_expression(state, p, PERCENT_CHAR);
...
void g_assignment_expression(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, UNARY_EXPRESSION, ASSIGNMENT_OPERATOR, ASSIGNMENT_EXPRESSION)){
		g_unary_expression(first_child(p), state);      /*  This loads what we're going to assign to */
		g_assignment_expression(third_child(p), state); /*  Compute the value of what we want to assign */
		g_assignment_operator(second_child(p), state);  /*  Compute the result of any assignment operator */
		do_assignment(state, p);
...
void g_expression_rest(struct parser_node * p, struct code_gen_state * state){
...
void g_expression(struct parser_node * p, struct code_gen_state * state){
...
void g_constant_expression(struct parser_node * p, struct code_gen_state * state){
...
void g_expression_statement(struct parser_node * p, struct code_gen_state * state){
	if(is_first_child_type(p, TERMINAL)){
		...
		if(is_terminal_c_token_type(second_child(p),SEMICOLON_CHAR)){
			g_expression(first_child(p), state);
...
void g_statement(struct parser_node * p, struct code_gen_state * state, unsigned char * evaluate_label, unsigned char * end_label){
	if(is_first_child_type(p, LABELED_STATEMENT)){
		g_labeled_statement(first_child(p), state);
	}else if(is_first_child_type(p, COMPOUND_STATEMENT)){
		g_compound_statement(first_child(p), state, (struct parser_node *)0, evaluate_label, end_label);
	}else if(is_first_child_type(p, EXPRESSION_STATEMENT)){
		g_expression_statement(first_child(p), state);
	}else if(is_first_child_type(p, SELECTION_STATEMENT)){
		g_selection_statement(first_child(p), state);
	}else if(is_first_child_type(p, ITERATION_STATEMENT)){
		g_iteration_statement(first_child(p), state);
	}else if(is_first_child_type(p, JUMP_STATEMENT)){
		g_jump_statement(first_child(p), state);
	}else{
		buffered_printf(state->buffered_output,"Unsupported statement type is: %d\n",first_child(p)->type);
	}
}

void g_statement_list_rest(struct parser_node * p, struct code_gen_state * state){
...
void clear_locals_from_scope(struct code_gen_state * state, struct scope_level * l){
	...
	if(l->first_local_object){
		...
		while(obj){
			manage_local_space(state, obj, 0);
			obj = obj->next;
		}
	}
}
...
void create_default_return_value(struct code_gen_state * state, struct parser_node * possible_function, struct parser_node * context){
	...
	convert_to_untypedefed_type_description(state->memory_pool_collection, return_type_description);
	rtn_val_size = type_size(state, return_type_description, return_type_description->value_type, 1, state->current_scope);
	...
	push_type(state, copy_type_description(state->memory_pool_collection, return_type_description), context);
...
void do_default_function_return(struct code_gen_state * state, struct parser_node * possible_function, struct parser_node * context){
	if(!possible_function){
		return;
	}
	create_default_return_value(state, possible_function, context);
	return_from_function(state, possible_function);
}
...
void g_compound_statement(struct parser_node * p, struct code_gen_state * state, struct parser_node * possible_function, unsigned char * evaluate_label, unsigned char * end_label){
	...
	if(check_four_children(p, TERMINAL, DECLARATION_LIST, STATEMENT_LIST, TERMINAL)){
		if(
			...
			g_declaration_list(second_child(p), state);
			g_statement_list(third_child(p), state);
			do_default_function_return(state, possible_function, fourth_child(p));
			if(evaluate_label){
		...
		if(
			...
			g_declaration_list(second_child(p), state);
			do_default_function_return(state, possible_function, third_child(p));
			if(evaluate_label){
				unsigned_char_ptr_list_pop_end(&scope->evaluate_labels);
			}
			if(end_label){
		...
		if(
			...
			g_statement_list(second_child(p), state);
			do_default_function_return(state, possible_function, third_child(p));
			if(evaluate_label){
				unsigned_char_ptr_list_pop_end(&scope->evaluate_labels);
			}
		...
		if(
			...
			do_default_function_return(state, possible_function, second_child(p));
			if(evaluate_label){
				unsigned_char_ptr_list_pop_end(&scope->evaluate_labels);
			}
			if(end_label){
...
void g_storage_class_specifier(struct parser_node * p, struct code_gen_state * state){
	if(is_first_child_type(p, TERMINAL)){
...
void g_enumerator(struct parser_node * p, struct code_gen_state * state){
...
void g_enumerator_list_rest(struct parser_node * p, struct code_gen_state * state){
...
void g_enumerator_list(struct parser_node * p, struct code_gen_state * state){
...
void g_enum_specifier(struct parser_node * p, struct code_gen_state * state){
...
void g_struct_or_union(struct parser_node * p, struct code_gen_state * state){
	if(is_first_child_type(p, TERMINAL)){
...
void g_type_specifier(struct parser_node * p, struct code_gen_state * state){
	if(is_first_child_type(p, TERMINAL)){
		...
		g_enum_specifier(first_child(p), state);
	}else if(is_first_child_type(p, STRUCT_OR_UNION_SPECIFIER)){
		g_struct_or_union_specifier(first_child(p), state);
	}else{
		assert(0 &&"Expected terminal or enum_specifier.\n");
	}
}

void g_declaration_specifiers(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, STORAGE_CLASS_SPECIFIER, DECLARATION_SPECIFIERS)){
		g_storage_class_specifier(first_child(p), state);
		g_declaration_specifiers(second_child(p), state);
	}else if(check_two_children(p, TYPE_SPECIFIER, DECLARATION_SPECIFIERS)){
		g_type_specifier(first_child(p), state);
		g_declaration_specifiers(second_child(p), state);
	}else if(check_two_children(p, TYPE_QUALIFIER, DECLARATION_SPECIFIERS)){
		g_type_qualifier(first_child(p), state);
		g_declaration_specifiers(second_child(p), state);
...
void g_init_declarator_list_rest(struct parser_node * specifiers, struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, INIT_DECLARATOR, INIT_DECLARATOR_LIST_REST)){
		if(is_terminal_c_token_type(first_child(p),COMMA_CHAR)){
			g_init_declarator(specifiers, second_child(p), state);
			g_init_declarator_list_rest(specifiers, third_child(p), state);
		}else{
			assert(0 &&"Expected COMMA_CHAR.\n");
		}
	}else if(check_one_child(p, EPSILON)){
...
void g_init_declarator_list(struct parser_node * specifiers, struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, INIT_DECLARATOR,INIT_DECLARATOR_LIST_REST)){
		g_init_declarator(specifiers, first_child(p), state);
		g_init_declarator_list_rest(specifiers, second_child(p), state);
	}else{
		assert(0 &&"Expected init_declarator_list.\n");
	}
}

void g_declaration(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, DECLARATION_SPECIFIERS,INIT_DECLARATOR_LIST, TERMINAL)){
		if(is_terminal_c_token_type(third_child(p),SEMICOLON_CHAR)){
			g_declaration_specifiers(first_child(p), state);
			g_init_declarator_list(first_child(p), second_child(p), state);
		}else{
			assert(0 &&"Expected terminal.\n");
		}
...
void g_declaration_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, DECLARATION, DECLARATION_LIST_REST)){
		g_declaration(first_child(p), state);
		g_declaration_list_rest(second_child(p), state);
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
		assert(0 &&"Expected DECLARATION.\n");
...
void g_declaration_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, DECLARATION, DECLARATION_LIST_REST)){
		g_declaration(first_child(p), state);
		g_declaration_list_rest(second_child(p), state);
	}else{
...
void g_identifier_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, TERMINAL, IDENTIFIER_LIST_REST)){
		if(
			is_terminal_c_token_type(first_child(p),COMMA_CHAR) &&
			is_terminal_c_token_type(second_child(p),IDENTIFIER)
		){
			g_identifier_list_rest(third_child(p), state);
		}else{
			assert(0 &&"Expected COMMA_CHAR.\n");
		}
	}else if(check_one_child(p, EPSILON)){
...
void g_identifier_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, TERMINAL, IDENTIFIER_LIST_REST)){
		if(is_terminal_c_token_type(first_child(p),IDENTIFIER)){
			g_identifier_list_rest(second_child(p), state);
		}else{
			assert(0 &&"Expected COMMA_CHAR.\n");
		}
	}else{
...
void g_direct_declarator_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, CONSTANT_EXPRESSION, TERMINAL, DIRECT_DECLARATOR_REST)){
		if(
			...
			g_direct_declarator_rest(fourth_child(p), state);
		}else{
		...
		if(
			...
			g_parameter_type_list(second_child(p), state);
			g_direct_declarator_rest(fourth_child(p), state);
		}else{
			assert(0 &&"Expected direct_declarator rest.\n");
		}
		...
		if(
			...
			g_identifier_list(second_child(p), state);
			g_direct_declarator_rest(fourth_child(p), state);
		}else{
			assert(0 &&"Expected direct_declarator rest.\n");
		}
		...
		if(
			...
			g_direct_declarator_rest(third_child(p), state);
		} else if(
			is_terminal_c_token_type(first_child(p),OPEN_PAREN_CHAR) &&
			is_terminal_c_token_type(second_child(p),CLOSE_PAREN_CHAR)
		){
...
void g_type_qualifier_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, TYPE_QUALIFIER, TYPE_QUALIFIER_LIST_REST)){
		g_type_qualifier(first_child(p), state);
		g_type_qualifier_list_rest(second_child(p), state);
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
...
void g_type_qualifier_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, TYPE_QUALIFIER, TYPE_QUALIFIER_LIST_REST)){
		g_type_qualifier(first_child(p), state);
		g_type_qualifier_list_rest(second_child(p), state);
	}else{
...
void g_pointer(struct parser_node * p, struct code_gen_state * state){
	if(is_terminal_c_token_type(first_child(p),MULTIPLY_CHAR)) {
		if(check_three_children(p, TERMINAL, TYPE_QUALIFIER_LIST, POINTER)){
			g_type_qualifier_list(second_child(p), state);
			g_pointer(third_child(p), state);
...
void manage_local_space(struct code_gen_state * state, struct namespace_object * obj, unsigned is_initializing){
	...
	unsigned int obj_size = get_namespace_object_size(state, obj, 0);
	arity = obj_size / 4;
	assert((obj_size - (arity * 4)) == 0);
	for(i = 0; i < arity; i++){
		buffered_printf(state->buffered_output,"%s SP SP WR; Room for local val\n", instruction);
	}
}

void g_direct_declarator(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, DECLARATOR, TERMINAL, DIRECT_DECLARATOR_REST)){
		if(
			is_terminal_c_token_type(first_child(p),OPEN_PAREN_CHAR) &&
			is_terminal_c_token_type(third_child(p),CLOSE_PAREN_CHAR)
		){
			g_declarator(second_child(p), state);
			g_direct_declarator_rest(fourth_child(p), state);
		}
		...
		if(
			...
			unsigned char * identifier = copy_string(first_child(p)->c_lexer_token->first_byte, first_child(p)->c_lexer_token->last_byte, state->memory_pool_collection);
			load_identifier(state, identifier, p);
			heap_memory_pool_free(state->memory_pool_collection, identifier);
			g_direct_declarator_rest(second_child(p), state);
		}
	}else{
...
void g_declarator(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, POINTER, DIRECT_DECLARATOR)){
		g_pointer(first_child(p), state);
		g_direct_declarator(second_child(p), state);
	}else if(check_one_child(p, DIRECT_DECLARATOR)){
		g_direct_declarator(first_child(p), state);
	}else{
		assert(0 &&"Expected pointer or direct_declarator.\n");
	}
}

void g_initializer_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, INITIALIZER, INITIALIZER_LIST_REST)){
		g_initializer(second_child(p), state);
		g_initializer_list_rest(third_child(p), state);
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
...
void g_initializer_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, INITIALIZER, INITIALIZER_LIST_REST)){
		g_initializer(first_child(p), state);
		g_initializer_list_rest(second_child(p), state);
	}else{
		assert(0 &&"Expected initializer.\n");
	}
}

void g_initializer(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, INITIALIZER_LIST, TERMINAL, TERMINAL)){
		if(
			...
			g_initializer_list(second_child(p), state);
		}
...
struct type_description * manage_assignment_type_change(struct code_gen_state * state, struct parser_node * context){
	...
	if(is_arithmetic_type(state->memory_pool_collection, t2)){
		enum type_class tc1 = determine_type_class(state->memory_pool_collection, t1);
		enum type_class tc2 = determine_type_class(state->memory_pool_collection, t2);
		if(tc1 == TYPE_CLASS_CHAR || tc1 == TYPE_CLASS_SHORT){
			perform_integral_promotion(state, &t1, "SP");
		}
		if(tc2 == TYPE_CLASS_CHAR || tc2 == TYPE_CLASS_SHORT){
			convert_top_rvalue_to_target_type(state, t1, t2);
			destroy_type_description(state->memory_pool_collection, t1);
			return t2;
		}else{
			push_type(state, t2, context); /*  Poped inside usual_arithmetic_conversion */
			push_type(state, t1, context);
			return usual_arithmetic_conversion(state, context);
		}
	}else if(determine_type_class(state->memory_pool_collection, t2) == TYPE_CLASS_POINTER){
...
void g_init_declarator(struct parser_node * specifiers, struct parser_node * p, struct code_gen_state * state){
	struct memory_pool_collection * m = state->memory_pool_collection;
	...
	*/
		...
		if(check_three_children(p, DECLARATOR, TERMINAL, INITIALIZER)){
			if(is_terminal_c_token_type(second_child(p),EQUALS_CHAR)){
				if(is_global){
					if(!is_extern && !is_typedef){
						...
						buffered_printf(state->buffered_output,"%s:\n", name);
						process_global_type(state, &constants, &types);
						buffered_printf(state->buffered_output,"%s:\n", end);
			...
			if(is_global){
				if(!is_extern && !is_typedef){
					...
					buffered_printf(state->buffered_output,"SW 0x%X;\n", size / 4);
					buffered_printf(state->buffered_output,"%s:\n", end);
					implement_external_symbol(m, &state->symbols, name);
					implement_external_symbol(m, &state->symbols, end);
					/*  Remember this function */
	...
	destroy_parser_node_tree_and_c_lexer_tokens(m, abstract_declarator);
	destroy_type_description(m, type_description);
	heap_memory_pool_free(m, identifier_str);
}

void g_struct_declarator(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, DECLARATOR, TERMINAL, CONSTANT_EXPRESSION)){
		if(is_terminal_c_token_type(second_child(p),COLON_CHAR)){
			g_declarator(first_child(p), state);
			/*g_constant_expression(third_child(p), state);*/
		}
...
void g_struct_declarator_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, STRUCT_DECLARATOR, STRUCT_DECLARATOR_LIST_REST)){
		if(is_terminal_c_token_type(first_child(p),COMMA_CHAR)){
			g_struct_declarator(second_child(p), state);
			g_struct_declarator_list_rest(third_child(p), state);
		}
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
...
void g_struct_declarator_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, STRUCT_DECLARATOR, STRUCT_DECLARATOR_LIST_REST)){
		g_struct_declarator(first_child(p), state);
		g_struct_declarator_list_rest(second_child(p), state);
	}else{
		assert(0 &&"Expected struct_declarator\n");
	}
}

void g_specifier_qualifier_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, TYPE_SPECIFIER, SPECIFIER_QUALIFIER_LIST)){
		g_type_specifier(first_child(p), state);
		g_specifier_qualifier_list(second_child(p), state);
...
void g_struct_declaration(struct parser_node * p, struct code_gen_state * state){
	(void)state;
	if(check_three_children(p, SPECIFIER_QUALIFIER_LIST, STRUCT_DECLARATOR_LIST, TERMINAL)){
		if(is_terminal_c_token_type(third_child(p),SEMICOLON_CHAR)){
			/*g_specifier_qualifier_list(first_child(p), state);*/
			/*g_struct_declarator_list(second_child(p), state);*/
		}
	}else{
...
void g_struct_declaration_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, STRUCT_DECLARATION, STRUCT_DECLARATION_LIST_REST)){
		g_struct_declaration(first_child(p), state);
		g_struct_declaration_list_rest(second_child(p), state);
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
...
void g_struct_declaration_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, STRUCT_DECLARATION, STRUCT_DECLARATION_LIST_REST)){
		g_struct_declaration(first_child(p), state);
		g_struct_declaration_list_rest(second_child(p), state);
	}else{
...
void g_struct_or_union_specifier(struct parser_node * p, struct code_gen_state * state){
	if(check_five_children(p, STRUCT_OR_UNION, TERMINAL, TERMINAL, STRUCT_DECLARATION_LIST, TERMINAL)){
		if(
			...
			g_struct_or_union(first_child(p), state);
			g_struct_declaration_list(fourth_child(p), state);
		}else{
		...
		if(
			...
			g_struct_or_union(first_child(p), state);
			g_struct_declaration_list(third_child(p), state);
		}else{
			assert(0 &&"Expected struct_or_union_specifier\n");
		}
...
void g_parameter_declaration(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, DECLARATION_SPECIFIERS, ABSTRACT_DECLARATOR)){
		g_declaration_specifiers(first_child(p), state);
		g_abstract_declarator(second_child(p), state);
...
void g_parameter_list_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, PARAMETER_DECLARATION, PARAMETER_LIST_REST)){
		if(is_terminal_c_token_type(first_child(p),COMMA_CHAR)){
			g_parameter_declaration(second_child(p), state);
			g_parameter_list_rest(third_child(p), state);
		}else{
...
void g_parameter_list(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, PARAMETER_DECLARATION, PARAMETER_LIST_REST)){
		g_parameter_declaration(first_child(p), state);
		g_parameter_list_rest(second_child(p), state);
	}else{
		assert(0 &&"Expected parameter parameter_list.\n");
	}
}

void g_parameter_type_list(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, PARAMETER_LIST, TERMINAL, TERMINAL)){
		if(
			...
			g_parameter_list(first_child(p), state);
		}else{
			assert(0 &&"Expected ,....\n");
		}
...
void g_direct_abstract_declarator_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, CONSTANT_EXPRESSION, TERMINAL, DIRECT_ABSTRACT_DECLARATOR_REST)){
		if(
			...
			/*  Not evaluating expression here, because I don't think its necessary and it loads a type we don't want */
			/*  In the future this might be necessary to calculate at run time for variables sized arrays */
			g_direct_abstract_declarator_rest(fourth_child(p), state);
		}else{
			assert(0 &&"Expected []");
		}
		...
		if(
			...
			g_direct_abstract_declarator_rest(third_child(p), state);
		}else if(
			is_terminal_c_token_type(first_child(p),OPEN_PAREN_CHAR) &&
			is_terminal_c_token_type(second_child(p),CLOSE_PAREN_CHAR)
		){
...
void g_direct_abstract_declarator(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, TERMINAL, CONSTANT_EXPRESSION, TERMINAL, DIRECT_ABSTRACT_DECLARATOR_REST)){
		if(
			is_terminal_c_token_type(first_child(p),OPEN_SQUARE_BRACKET_CHAR) &&
			is_terminal_c_token_type(third_child(p),CLOSE_SQUARE_BRACKET_CHAR)
...
void g_abstract_declarator(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, POINTER, DIRECT_ABSTRACT_DECLARATOR)){
		g_pointer(first_child(p), state);
		g_direct_abstract_declarator(second_child(p), state);
	}else if(check_one_child(p, POINTER)){
		g_pointer(first_child(p), state);
	}else if(check_one_child(p, DIRECT_ABSTRACT_DECLARATOR)){
		g_direct_abstract_declarator(first_child(p), state);
	}else{
...
void g_labeled_statement(struct parser_node * p, struct code_gen_state * state){
	...
	if(check_four_children(p, TERMINAL, CONSTANT_EXPRESSION, TERMINAL, STATEMENT)){
		if(
			...
			assert(struct_switch_frame_ptr_list_size(&state->switch_frames));
			frame = struct_switch_frame_ptr_list_get(&state->switch_frames, struct_switch_frame_ptr_list_size(&state->switch_frames)-1);
			assert(second_child(p)->type == CONSTANT_EXPRESSION);
			switch_value = evaluate_compile_time_constant(state, evaluate_constant_constant_expression(state, second_child(p)));
			...
			buffered_printf(state->buffered_output,"%s:\n", case_str);
			implement_internal_symbol(m, &state->symbols, case_str);
			heap_memory_pool_free(m, case_str);
			g_statement(fourth_child(p), state, (unsigned char *)0, (unsigned char *)0);
		}else{
		...
		if(
			...
			buffered_printf(state->buffered_output,"%s:\n", default_str);
			implement_internal_symbol(m, &state->symbols, default_str);
			g_statement(third_child(p), state, (unsigned char *)0, (unsigned char *)0);
			heap_memory_pool_free(m, default_str);
		}else if(
...
void find_child_case_labels_h(struct code_gen_state * state, struct parser_node * p, struct switch_frame * frame){
	...
	if(n){
		...
		if(n->type == SELECTION_STATEMENT && first_child(n)->c_lexer_token->type == IF){
			find_child_case_labels_h(state, fifth_child(n), frame);
			if(p->first_child->next->next->next->next->next){
				find_child_case_labels_h(state, seventh_child(n), frame);
			}
		}else if(n->type == COMPOUND_STATEMENT){
			find_child_case_labels_h(state, second_child(n), frame);
			if(p->first_child->next){
				find_child_case_labels_h(state, third_child(n), frame);
			}
		}else if(n->type == ITERATION_STATEMENT){
			find_child_case_labels_h(state, second_child(n), frame);
			find_child_case_labels_h(state, fifth_child(n), frame);
			if(p->first_child->next->next->next->next){
				find_child_case_labels_h(state, sixth_child(n), frame);
			}
			if(p->first_child->next->next->next->next->next){
				find_child_case_labels_h(state, seventh_child(n), frame);
			}
		}else if(n->type == STATEMENT){
			find_child_case_labels_h(state, first_child(n), frame);
			...
			if(n->first_child->c_lexer_token->type == CASE){
				...
				size = evaluate_compile_time_constant(state, evaluate_constant_constant_expression(state, n->first_child->next));
				unsigned_int_list_add_end(&frame->values, size);
				find_child_case_labels_h(state, fourth_child(n), frame);
...
void find_child_case_labels(struct code_gen_state * state, struct parser_node * p, struct switch_frame * frame){
	assert(p->type == STATEMENT);
	frame->has_default = 0;
	unsigned_int_list_create(&frame->values);
	find_child_case_labels_h(state, p->first_child, frame);
}

void g_selection_statement(struct parser_node * p, struct code_gen_state * state){
	struct memory_pool_collection * m = state->memory_pool_collection;
	if(check_seven_children(p, TERMINAL, TERMINAL, EXPRESSION, TERMINAL, STATEMENT, TERMINAL, STATEMENT)){
		...
		if(
			...
			for(i = 0; i < num_cases; i++){
				...
				buffered_printf(state->buffered_output,"ll r2 0x%X;\n", value);
				buffered_printf(state->buffered_output,"beq r1 r2 %s;\n", case_str);
				require_internal_symbol(m, &state->symbols, case_str);
				heap_memory_pool_free(m, case_str);
			}
			...
			if(uses_compound_statement){
				...
				heap_memory_pool_free(m, unsigned_char_ptr_list_pop_end(&scope->end_labels));
			}
		}else{
...
void g_iteration_statement(struct parser_node * p, struct code_gen_state * state){
	struct memory_pool_collection * m = state->memory_pool_collection;
	if(check_seven_children(p, TERMINAL, STATEMENT, TERMINAL, TERMINAL, EXPRESSION, TERMINAL, TERMINAL)){
		if(
			...
			require_internal_symbol(m, &state->symbols, endwhile_str);
			implement_internal_symbol(m, &state->symbols, evaluatewhile_str);
			implement_internal_symbol(m, &state->symbols, startwhile_str);
			implement_internal_symbol(m, &state->symbols, endwhile_str);
			heap_memory_pool_free(m, startwhile_str);
			if(uses_compound_statement){
				heap_memory_pool_free(m, evaluatewhile_str);
				heap_memory_pool_free(m, endwhile_str);
			}else{
				heap_memory_pool_free(m, unsigned_char_ptr_list_pop_end(&scope->evaluate_labels));
				heap_memory_pool_free(m, unsigned_char_ptr_list_pop_end(&scope->end_labels));
			}
		}else{
		...
		if(
			...
			(void)scope;
			state->condition_index = state->condition_index + 1;
			g_expression_statement(third_child(p), state);
			buffered_printf(state->buffered_output,"%s:\n", startfor_str);
			...
			require_internal_symbol(m, &state->symbols, endfor_str);
			require_internal_symbol(m, &state->symbols, evaluatefor_str);
			implement_internal_symbol(m, &state->symbols, startfor_str);
			implement_internal_symbol(m, &state->symbols, endfor_str);
			implement_internal_symbol(m, &state->symbols, evaluatefor_str);
			heap_memory_pool_free(m, startfor_str);
			if(uses_compound_statement){
				heap_memory_pool_free(m, evaluatefor_str);
				heap_memory_pool_free(m, endfor_str);
			}else{
				heap_memory_pool_free(m, unsigned_char_ptr_list_pop_end(&scope->evaluate_labels));
				heap_memory_pool_free(m, unsigned_char_ptr_list_pop_end(&scope->end_labels));
			}
...
void g_jump_statement(struct parser_node * p, struct code_gen_state * state){
	if(check_three_children(p, TERMINAL, EXPRESSION, TERMINAL)){
		if(
		...
		if(
			...
			create_default_return_value(state, (struct parser_node *)0, first_child(p));
			return_from_function(state, p);
		}else{
...
void g_function_definition(struct parser_node * p, struct code_gen_state * state){
	if(check_four_children(p, DECLARATION_SPECIFIERS, DECLARATOR, DECLARATION_LIST, COMPOUND_STATEMENT)){
		g_declaration_specifiers(first_child(p), state);
		/*g_declarator(second_child(p), state);*/
		/*g_declaration_list(third_child(p), state);*/
		g_compound_statement(fourth_child(p), state, p, (unsigned char *)0, (unsigned char *)0);
		...
		g_declaration_specifiers(first_child(p), state);
		/*g_declarator(second_child(p), state);*/
		g_compound_statement(third_child(p), state, p, (unsigned char *)0, (unsigned char *)0);
		buffered_printf(state->buffered_output,"%s:\n", end);
		add_linker_object(state, identifier_str, end, L2_FUNCTION);
		...
		/*g_declarator(first_child(p), state);*/
		g_compound_statement(second_child(p), state, p, (unsigned char *)0, (unsigned char *)0);
	}else{
...
void g_external_declaration(struct parser_node * p, struct code_gen_state * state){
	if(check_one_child(p, FUNCTION_DEFINITION)){
		g_function_definition(first_child(p), state);
...
void g_translation_unit_rest(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, EXTERNAL_DECLARATION, TRANSLATION_UNIT_REST)){
		g_external_declaration(first_child(p), state);
		g_translation_unit_rest(second_child(p), state);
	}else if(check_one_child(p, EPSILON)){
		/*  Do nothing */
	}else{
		assert(0 &&"Expected function declaration or declaration.\n");
...
void g_translation_unit(struct parser_node * p, struct code_gen_state * state){
	if(check_two_children(p, EXTERNAL_DECLARATION, TRANSLATION_UNIT_REST)){
		unsigned int i;
		struct struct_constant_description_ptr_list constants = unsigned_char_ptr_to_struct_constant_description_ptr_map_values(&state->parser_state->constant_map);
		...
		for(i = 0; i < num_string_literals; i++){
			...
			buffered_printf(state->buffered_output,"%s:\n", end);
			implement_internal_symbol(state->memory_pool_collection, &state->symbols, string_literal_identifier_str);
			implement_internal_symbol(state->memory_pool_collection, &state->symbols, end);

			add_linker_object(state, string_literal_identifier_str, end, L2_CONSTANT);
		}
...
struct linker_symbol * make_linker_symbol(struct memory_pool_collection * m, unsigned int is_impl, unsigned int is_req, unsigned int is_ext){
	struct linker_symbol * new_symbol = struct_linker_symbol_memory_pool_malloc(m);
...
void require_internal_symbol(struct memory_pool_collection * m, struct unsigned_char_ptr_to_struct_linker_symbol_ptr_map * map, unsigned char * c){
	...
	if(existing_symbol){
		...
		unsigned_char_ptr_to_struct_linker_symbol_ptr_map_put(map, copy_string(c, get_null_terminator(c), m), make_linker_symbol(m, 0, 1, 0));
	}
}
...
void implement_internal_symbol(struct memory_pool_collection * m, struct unsigned_char_ptr_to_struct_linker_symbol_ptr_map * map, unsigned char * c){
	...
	if(existing_symbol){
		...
		unsigned_char_ptr_to_struct_linker_symbol_ptr_map_put(map, copy_string(c, get_null_terminator(c), m), make_linker_symbol(m, 1, 0, 0));
	}
}
...
void implement_external_symbol(struct memory_pool_collection * m, struct unsigned_char_ptr_to_struct_linker_symbol_ptr_map * map, unsigned char * c){
	...
	if(existing_symbol){
		...
		unsigned_char_ptr_to_struct_linker_symbol_ptr_map_put(map, copy_string(c, get_null_terminator(c), m), make_linker_symbol(m, 1, 0, 1));
	}
}
...
void require_external_symbol(struct memory_pool_collection * m, struct unsigned_char_ptr_to_struct_linker_symbol_ptr_map * map, unsigned char * c){
	...
	if(existing_symbol){
		...
		unsigned_char_ptr_to_struct_linker_symbol_ptr_map_put(map, copy_string(c, get_null_terminator(c), m), make_linker_symbol(m, 0, 1, 1));
	}
}

int generate_code(struct code_gen_state * state){
	if(state->parser_state->top_node){
		if(state->parser_state->top_node->type == TRANSLATION_UNIT){
			g_translation_unit(state->parser_state->top_node, state);
		}else{
...
int destroy_code_gen_state(struct code_gen_state * state){
	struct unsigned_char_ptr_list keys = unsigned_char_ptr_to_struct_linker_symbol_ptr_map_keys(&state->symbols);
	unsigned int size = unsigned_char_ptr_list_size(&keys);
	unsigned int i;
	for(i = 0; i < size; i++){
		struct_linker_symbol_memory_pool_free(state->memory_pool_collection, unsigned_char_ptr_to_struct_linker_symbol_ptr_map_get(&state->symbols, unsigned_char_ptr_list_get(&keys, i)));
	}
	...
	/*  Intitializer levels */
	for(i = 0; i < struct_constant_initializer_level_ptr_list_size(&state->created_constant_initializer_levels); i++){
		struct constant_initializer_level * l = struct_constant_initializer_level_ptr_list_get(&state->created_constant_initializer_levels, i);
		struct_constant_initializer_level_ptr_list_destroy(&l->children);
		free(l);
	}
	struct_constant_initializer_level_ptr_list_destroy(&state->created_constant_initializer_levels);

	/*  Constant descriptions */
	for(i = 0; i < struct_constant_description_ptr_list_size(&state->created_constant_descriptions); i++){
		free(struct_constant_description_ptr_list_get(&state->created_constant_descriptions, i));
	}
	struct_constant_description_ptr_list_destroy(&state->created_constant_descriptions);

	/*  Compile time constants */
	for(i = 0; i < struct_compile_time_constant_ptr_list_size(&state->created_compile_time_constants); i++){
		free(struct_compile_time_constant_ptr_list_get(&state->created_compile_time_constants, i));
	}
	struct_compile_time_constant_ptr_list_destroy(&state->created_compile_time_constants);

	/*  For 'native_data' members */
	for(i = 0; i < void_ptr_list_size(&state->created_data); i++){
		free(void_ptr_list_get(&state->created_data, i));
	}
	void_ptr_list_destroy(&state->created_data);

	for(i = 0; i < struct_linker_object_ptr_list_size(&state->object_declarations); i++){
		struct linker_object * obj = struct_linker_object_ptr_list_get(&state->object_declarations, i);
	...
	struct_linker_object_ptr_list_destroy(&state->object_declarations);
	return 0;
}
	...
	if(rtn){
		...
		if(parse(&parser_state) || (struct_parser_error_list_size(&parser_state.parser_errors) != 0)){
			...
			if(generate_code(&state)){
				...
				unsigned int num_objects = struct_linker_object_ptr_list_size(&state.object_declarations);
				unsigned int k;
				struct unsigned_char_ptr_list keys = unsigned_char_ptr_to_struct_linker_symbol_ptr_map_keys(&state.symbols);
				unsigned int labels_size = unsigned_char_ptr_list_size(&keys);
				buffered_printf(state.buffered_symbol_table, "OFFSET %s;\n", offset);
				for(k = 0; k < num_objects; k++){
					struct linker_object * obj = struct_linker_object_ptr_list_get(&state.object_declarations, k);
					switch(obj->type){
						case L2_FUNCTION:{
