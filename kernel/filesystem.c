...
int do_compile(void){
	while(1){} /*  Make this task do nothing for now. */
	while(1){
		...
		create_parser_state(&parser_state, &memory_pool_collection, &c_lexer_state, &generated_code, unsigned_char_list_data(&preprocssed_characters), &asm);
